    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between gap-4">
        <Link to="/" className="flex items-center gap-2 font-bold">
          <div className="grid size-8 place-items-center rounded-md bg-primary text-primary-foreground">MOSSD</div>
    ...
    <Sheet>
      <SheetTrigger asChild>
        {asButton ? (
          ...
          <button className="px-3 py-2 text-sm font-medium rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground">About Us</button>
        )}
      </SheetTrigger>
      <SheetContent side="right" className="w-[420px]">
        <SheetHeader>
          <SheetTitle>Về nhà hàng</SheetTitle>
          <SheetDescription>Tổng quan về concept, chất lượng món ăn và dịch vụ.</SheetDescription>
        </SheetHeader>
        <div className="mt-4 space-y-3 text-sm text-muted-foreground">
          <p>Nhà hàng tập trung vào nguyên liệu tươi, chế biến theo ngày. Không gian hiện đại, phù hợp gia đình và bạn bè.</p>
          <p>Giờ mở cửa: 10:00 - 22:00 hằng ngày. Đặt bàn qua mục Reservation để giữ chỗ.</p>
        </div>
      </SheetContent>
    </Sheet>
  );
    ...
    <Sheet>
      <SheetTrigger asChild>
        {asButton ? (
          ...
          <button className="px-3 py-2 text-sm font-medium rounded-md text-muted-foreground hover:bg-accent hover:text-accent-foreground">Reservation</button>
        )}
      </SheetTrigger>
      <SheetContent side="right" className="w-[420px]">
        <SheetHeader>
